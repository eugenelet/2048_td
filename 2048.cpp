...
public:
	...
	state select_best_move(const board& b) const {
		// TODO: select by estimated value
		state after[4] = { { b, 0 }, { b, 1 }, { b, 2 }, { b, 3 } }; // up, right, down, left
        for(int i=0; i<4; i++){
            after[i].set_value(after[i].reward() + estimate(after[i].after_state()) );
            if(after[i].reward() < 0){
            	after[i].set_value(after[i].value() * after[i].reward());
            }
        }
		return *std::max_element(after, after + 4);
	}
	...
	void update_episode(std::vector<state>& path, const float& alpha = 0.1) const {
        ...
        state cur_state;
        state prev_state;
        cur_state = path[path.size() - 1];
        for(int i=path.size() - 2; i >= 0; i--){
            prev_state = path[i];
            update(prev_state.after_state(), alpha*(prev_state.reward() + cur_state.value() - prev_state.value() ));
...
int main(int argc, const char* argv[]) {
	...
	learning tdl;
    // debug.set_enable();
	// TODO: set the learning parameters
	float alpha = 0.1;
